...
unsigned tsflags ;
{
    ...
    if (src == NULL) {
        ...
        return(resize(tscpy, (ts-tscpy+2)*sizeof(TCHAR))) ;
 ...
 */
        if (dirflag)
          {
#ifdef DOSWIN32
        // Treat "" as path ending with '\'.  Returning '\*' is
        // wrong because  '\*' is INVALID
           if (c == PathChar || c == NULLC)
#else
 ...
 *      specified in fspec.
 *
 *  struct cpyinfo *SetFsSetSaveDir(TCHAR *fspec)
 *
 *  Args:
...
struct cpyinfo *SetFsSetSaveDir(fspec)
TCHAR *fspec ;
{
        TCHAR *tmpptr;
        TCHAR *buftemp;
        ...
        fsinfo = (struct cpyinfo *)gmkstr(sizeof(struct cpyinfo)) ; /*WARNING*/
