...
static int ParenFlag ;  /* > 0 if a paren statement group is being parsed */

extern struct batdata *CurBat ;         /* M001 - Added for Parsefor()     */
...
DuRd(struct node *n)
{
        ...
        while (tmp) {
                ...
                cmd_printf(TEXT("%s"), tmp->fname) ;

                tmp = tmp->nxt ;
        } ;

        cmd_printf(CrLf) ;                                      /* M018    */
}
#endif
 ...
 *
 *  struct node *Parser(unsigned dfvalue, int dpvalue, int fsarg)
 *
 *  Args:
...
int fsarg ;
{
        ...
        int retcode ;           /* setjmp()'s return code */
        unsigned GeToken() ;
        ...
        ParenFlag = 0 ;             /* Initialize the parenthesis flag  */
 ...
 *
 *  Notes:
 *      If a parenthesised statement group is being parsed, read tokens until
 *      the current token isn't NLN.  If the current token is now a right
 *      parenthesis, unget it and return null.  This will save the paren for
 *      the instance of ParseS5() that is looking for it.
 *
...
/*  M004 - The code below has been rewritten to remove any tests for and
 *         discarding of newlines in parenthetical statements.  This is
 *         handled in BinaryOperator.  Additional stripping of newlines
 *         is handled in ParseS5().
 */
        if (TokTyp == NLN && !ParenFlag) {
 ...
 */
                ...
                if (*TokBuf == LPOP) {                          /* M015    */
                        n->type = PARTYP ;
                        ParenFlag++ ;
                        DEBUG((PAGRP, PALVL, "PS5: Paren group begins, pf = %d", ParenFlag)) ;
...
/*  M015 ends   */
                ...
                if (GeToken(GT_RPOP) == RPOP) { /* M000 - Was GT_NORMAL    */
                        ParenFlag-- ;
                        DEBUG((PAGRP, PALVL, "PS5: Paren group ends, pf = %d", ParenFlag)) ;
                        return(n) ;
                } ;
...
struct cmdnode *n ;
{
        ...
        } else if (*(s+i) && *(s+i-1) && *(s+i+2)) {        /* s1==s2 */
                DEBUG((PAGRP, PALVL, "PARG: s1==s2  i = %d  toklen = %d  t-3 = %d", i, TokLen, TokLen-3)) ;
                *(n->cmdline+i) = NULLC ;
                n->argptr = gmkstr(TokLen*sizeof(TCHAR)) ;    /*WARNING*/
                mystrcpy(n->argptr, s+i+2) ;

        } else {
                s = GeTexTok(GT_EQOK) ;
                if (_tcscmp(s, EQSTR) == 0) {                /* s1 == s2 */
                        n->argptr = GeTexTok(GT_NORMAL) ;
                        DEBUG((PAGRP, PALVL, "PARG: s1 == s2")) ;

                } else if (TokLen >= 4 && *s == EQ && *(s+1) == EQ) {   /* s1 ==s2 */
                        mystrcpy(s, s+2) ;
                        n->argptr = s ;
                        DEBUG((PAGRP, PALVL, "PARG: s1 ==s2")) ;
 ...
 */
        if (_tcscmp(opstr, TokBuf) == 0 ||
           (ParenFlag > 0 && _tcscmp(opstr, CSSTR) == 0 && *TokBuf == NLN)) {
...
unsigned flag ;
{
        ...
        if(ParenFlag)                           /* M000 - If flg, look     */
                flag = flag | GT_RPOP ;         /* ...for match            */
